...
distributed through immutable GitHub Releases of this repository and are
intended to be archived under marker-specific Zenodo records. Every released
bundle is content-addressed by metadata committed at its tag.
...
describes a publication of that content. Database releases are published from
this repository. The first corrected COI v1 publication uses
...
After immutable publication, new biological content requires a new semantic
database version, reviewed provenance, and a separate RTBioScan
activation/state-compatibility decision.
...
After publication, download all release assets into one directory and run:
...
- `tests/` contains synthetic regression tests; it contains no sequence data
  from a release.
- GitHub Releases in this repository contain the large database artifacts.
