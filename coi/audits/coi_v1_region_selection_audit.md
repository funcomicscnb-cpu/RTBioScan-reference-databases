...
This audit records why sequences in the pre-correction COI reconstruction
differ from records carrying the same accession in the official COInr
2024-05-06 release. It documents an analysis input and does not define the
corrected-v1 release identity.
...
- Pre-correction 29-exclusion reconstruction, canonical serialization SHA-256
  `d0b3aca535fbadcd0da8dfc7218c08e7b4151c9562ffe3fd37baf6cdcaef1775`.

That reconstruction contains 791,404 records. Comparing records by accession
...
selected pre-correction substring were compared with the forward primer and
the reverse complement of the reverse primer. Both boundaries matched exactly for 29,511
...
The audit does not independently reconstruct the exact historical
pre-clustering pool, complete CD-HIT command line, or membership of individual
98% clusters because the original inputs, run log, and `.clstr` output are
unavailable. The maintainer's construction account is accepted as attested
history; the missing evidence is a permanent independent-replay limitation,
not a release hold. The later loose FASTA is a malformed post-deployment append
and cannot establish the construction pool. Marker scope is separately
resolved: sequence-level evidence confirms that all 15 deployed identifiers
which name other markers are outside COI and corrected v1 excludes them. See
the
[`July artifact chronology audit`](coi_v1_july_artifact_chronology.md) and
[`marker-scope audit`](coi_v1_marker_scope_audit.md). The 40,318 target-region
substrings require no additional release exclusion.
